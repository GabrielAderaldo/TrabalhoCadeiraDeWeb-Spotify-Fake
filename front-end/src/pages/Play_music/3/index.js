...
import logo from '../3/assets/playlistImg.jpg'
...
const playlistJson = db.playlist[2];
const dbJson = [];
playlistJson.musicId.forEach(element => {
    dbJson.push(db.songs[element])
});*/

class Play_music_3 extends Component {
    ...
    componentDidMount(){
        const songs = [];
        axios.get(`http://localhost:8080/playlist/trap`)
      .then(res => {
    ...
    render(){
        ...
        if(songs[3]){
            return (
                <section id="playmusic-3-page-container">
                    <Navbar />
                    <div id="content-wrap">
